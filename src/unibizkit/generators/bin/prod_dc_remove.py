...
_CONTENT = '''\
#!/usr/bin/python3
"""Remove this app's production deployment completely from the server.

DESTRUCTIVE: removes the containers, the data volumes (database and uploaded
documents are LOST), the app images (on the docker daemon and in the registry)
and the whole ~/ubk/<app> directory — including the .env secrets, so a later
publish will generate fresh secrets. The shared ubk-registry container and
~/ubk itself are left in place (other apps may use them).
...
Steps:
  1. Asks for confirmation unless -f/--force is given.
  2. `compose down -v` on the active compose file (containers + volumes).
  3. Removes every <app>/* image from the server's docker daemon.
  4. Deletes every <app>/* repository from the registry (manifests via the
     registry API, then garbage-collects the registry storage).
  5. Deletes ~/ubk/<app>.
"""
import argparse
import sys
...
if not args.force:
    answer = input(
        f"This removes {pc.APP_ID} from {srv}: containers, ALL DATA (db + documents), "
        "images and secrets. Are you sure? [y/N] "
    )
...
if pc.ssh(srv, f"test -e {remote_dir}/docker-compose.yml", check=False).returncode == 0:
    ...
    print("Taking the stack down (containers + volumes)...")
