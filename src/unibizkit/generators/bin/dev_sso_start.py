...
from ..dev_sso.constants import REALM, REALM_NAME, KC_PORT, KC_MGMT_PORT
...
def update_supabase_config(secret):
    config_path = root_dir / 'backend' / 'supabase' / 'config.toml'
    ...
    for i, line in enumerate(lines):
        stripped = line.strip()
        if stripped == '[auth.external.keycloak]':
            in_keycloak = True
        elif stripped.startswith('[') and in_keycloak:
            in_keycloak = False
        elif in_keycloak and stripped.startswith('secret'):
            found = True
...
def restart_supabase_if_running():
    backend_dir = root_dir / 'backend'
    if not (backend_dir / 'supabase').exists():
        return "missing"
    ...
    print("Restarting Supabase to apply updated SSO secret...")
    run(['npx', 'supabase', 'stop'], cwd=backend_dir, stdout=sys.stdout, stderr=sys.stderr)
    run(['npx', 'supabase', 'start'], cwd=backend_dir, stdout=sys.stdout, stderr=sys.stderr)
    return "restarted"


def print_instructions(client_secret, supabase_config_status, supabase_restart_status):
    sep = "=" * 60
    ...
    print()
    if supabase_config_status == "updated":
        print("  supabase/config.toml updated automatically with the Keycloak secret.")
        if supabase_restart_status == "restarted":
            print("  Supabase was restarted automatically so the new secret is already active.")
        elif supabase_restart_status == "stopped":
            print("  Supabase is not running, so no restart was needed.")
        else:
            print("  Supabase project not found, so no restart was performed.")
    elif supabase_config_status == "unchanged":
        print("  supabase/config.toml already had the correct Keycloak secret.")
    else:
        print("  supabase/config.toml not found — run python bin/dev-supabase-create.py first,")
        print("  then re-run this script so the secret is applied automatically.")
    print()
...
run([*COMPOSE, 'build'], cwd=sso_dir, stdout=sys.stdout, stderr=sys.stderr)

print("Starting SSO containers...")
run([*COMPOSE, 'up', '-d'], cwd=sso_dir, stdout=sys.stdout, stderr=sys.stderr)

wait_for_keycloak()
client_secret = configure_keycloak()
supabase_config_status = update_supabase_config(client_secret)
supabase_restart_status = "not_needed"
if supabase_config_status == "updated":
    supabase_restart_status = restart_supabase_if_running()
print_instructions(client_secret, supabase_config_status, supabase_restart_status)
"""
