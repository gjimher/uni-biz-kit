...
* `presentation.jsonc` + `presentation/` — UI config, custom pages and list-row addons → [Frontend.md](Frontend.md)
...
* **View concepts** — a declared SQL query instead of a table: the same generated list, read-only by construction, with each row linking to the record it stands for → [Backend.md](Backend.md#view-concepts-view)
...
* **Record history** — trigger-based versioning for aggregate roots and their `part_of` children, including m:n relations and versioned document metadata, with visual comparison and guarded form restoration → [Backend.md](Backend.md#record-history-and-versioning)
