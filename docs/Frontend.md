...
Addon names use lower-case snake case. Names beginning with `_` are reserved for framework-generated actions; for example, record versioning injects `_version_details` and `_version_revert` into the `_version` list, and workflow task assignment injects `_task_assign_to_me` into the assignable tasks list, generating the matching addon files. Unknown concepts, missing addon files and model-owned files using the reserved prefix fail generation. As with every frontend visibility rule, `visible` is only a usability affordance: the backend must authorize every read or mutation performed by `execute`.
...
Those same design tokens also theme the **backoffice**: the generated `presentation/style/theme.js` reads `ACCENT`/`INK`/`MUTED`/`BORDER`/`BG`/`FONT` from `style/auth.jsx` and builds the react-admin theme from them, so a model that rebrands its auth pages gets an admin UI in the same palette and typography — without touching any generated resource, form or layout. A model that wants a different admin look (and only that) provides its own `presentation/style/theme.js` exporting `adminTheme`.
