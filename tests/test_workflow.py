...
def test_workflow_state_permissions():
    ...
    try:
        with conn.cursor() as cur:
            ...
            cur.execute(f"""
                INSERT INTO "order" (customer, order_date, total_amount, shipping_address, state, security_owner_id)
                VALUES ({customer_id}, CURRENT_TIMESTAMP, 50.00, 'Initial Addr', 'initial', '{user1_id}')
                RETURNING id;
            """)
